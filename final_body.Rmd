...
library(ggfortify)
library(survival)
fit <- survfit(Surv(time, status) ~ rx, data=studyA)
autoplot(fit)
```


```{r}
fit <- survfit(Surv(time, status) ~ rx, data=studyB)
autoplot(fit)
```
...
This shows a 40% less reduction in the rate of cancer reoccurance using
